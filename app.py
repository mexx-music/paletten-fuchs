...
# Beim Import KEINE UI-Ausgabe! (Fehlertext nur merken)
try:
...
_SS_CANVAS_UID = "pf_canvas_uid"  # eindeutiger Canvas-Key-Suffix pro Session
_SS_COMP_OK    = "pf_component_ok"
...
def _ensure():
    ...
    if _SS_CANVAS_UID not in st.session_state: st.session_state[_SS_CANVAS_UID] = str(uuid.uuid4())[:6]
    if _SS_COMP_OK not in st.session_state:    st.session_state[_SS_COMP_OK]    = True
...
def render_manager(title: str = "Eigene Layouts (Presets-Editor)", show_expander: bool = True) -> List[Dict[str, Any]]:
    ...
    with ct:
        if st.session_state.get("_pf_disable_canvas") or (not _HAS_CANVAS) or (not st.session_state[_SS_COMP_OK]):
            st.info("Canvas nicht verfügbar oder deaktiviert." + (f" Grund: {_CANVAS_ERR}" if _CANVAS_ERR else ""))
            return []
        ...
        # Canvas rendern (try/except + eindeutiger Key)
        initial_json = {"version": "5.2.4", "objects": st.session_state[_SS_OBJS]}
        try:
            canvas_key = f"pf_canvas_{st.session_state[_SS_CANVAS_UID]}"
            ...
            st.session_state[_SS_COMP_OK] = True
        except Exception as e:
            st.error(f"Canvas-Komponente hat einen Fehler geworfen: {e!s}")
            st.session_state[_SS_COMP_OK] = False
            st.session_state["_pf_disable_canvas"] = True
            return []

        # Vor Buttons aktuellen Stand stabil übernehmen (match pfid + snap)
