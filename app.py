...
def estimate_axle_split(fabric_json: Dict[str, Any]) -> Tuple[int, int]:
    ...
    if total == 0:
        return 50, 50
    f = int(round(100 * front_score / total))
    return f, 100 - f
...
with st.sidebar:
    ...
    n_euro = st.number_input("Euro (120×80)", min_value=0, max_value=45, value=12, step=1, format="%d", key="n_euro")
    n_ind = st.number_input("Industrie (100×120 quer)", min_value=0, max_value=30, value=4, step=1, format="%d", key="n_ind")
    n_ibc = st.number_input("IBC (schwer)", min_value=0, max_value=30, value=0, step=1, format="%d", key="n_ibc")
...
with col_right:
    ...
    if canvas.json_data:
        ...
        st.markdown(f"**Achslast:** vorn {f_pct}% / hinten {b_pct}%")
