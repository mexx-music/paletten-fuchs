...
# Paletten Fuchs – Vier Varianten im 2×2-Grid (33 / 32 / 31 / 24)
# - Unicode/Monospace-Rendering
# - Feste Rasterung: 25 Längsblöcke, 4 Zeichen je Block
# - Immer sichtbar: 33, 32 (2x Einzel quer), 31 (1x Einzel quer), 24 (6x2 quer + 5x3 längs)
# - 2×2-Grid -> passt auch auf iPad/Portrait besser als 4 Spalten
...
# -----------------------------
st.set_page_config(page_title="Paletten Fuchs – 4 Varianten (Grid)", layout="wide")

# Schrift leicht verkleinern, Container verbreitern -> stabil nebeneinander
st.markdown("""
<style>
...
CHARS_PER_BLOCK = 4             # 4 Zeichen je Rasterblock (wie besprochen, fix)
...
def cm_to_raster(cm: int) -> int:
    # Stabil visualisieren: 80 cm -> 1 Raster, 120 cm -> 2 Raster
    if abs(cm - 80) < 1e-6:
        return 1
    if abs(cm - 120) < 1e-6:
        return 2
...
def layout_for_preset_euro(n: int, singles_front: int = 0) -> List[Dict]:
    ...
    - ggf. eine 2-quer-Reihe (2 Paletten), falls (rest-2)%3==0
...
# -----------------------------
st.title("🦊 Paletten Fuchs – Vier Varianten (2×2)")
...
# 24 Euro: festes Muster (6×2 quer + 5×3 längs)
...
st.caption(
    "Tipp: Falls die Spalten dennoch untereinander erscheinen, bitte auf **Querformat** drehen oder das Fenster verbreitern. "
    "Diese Ansicht ist bewusst schmaler (kleinere Schrift), damit zwei Spalten auch auf Tablets nebeneinander passen."
)
