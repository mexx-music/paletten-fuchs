...
st.set_page_config(page_title="🦊 PAL Fuchs 8 (auf Basis PF6)", layout="wide")
st.title("🦊 PAL Fuchs 8 – Draufsicht mit Icons (PF6-Logik)")
...
def span(name, ori):
    ...
    dx = max(1, depth_cm // cell_cm)   # entlang Länge
    dy = max(1, width_cm // cell_cm)   # quer
    return dx, dy
...
def fill_tail_closed_euro(x_start, euro_left):
    # bevorzugt 3er-Reihen längs; sonst 2 quer Abschluss
...
def industrie_all(n):
    ...
    if n%2==1:
        y=center_y(wq)
        if free(x,y,dq,wq): place_int = place(x,y,dq,wq, ICON[("Industrie","q")], "Industrie"); n-=1; x+=dq
